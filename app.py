...
SYSTEM_INSTRUCTION = (
    ...
    "Responde claramente al comando "
    "\"diagnóstico\", \"resumen\" o \"tratamientos\" según lo solicite el usuario."
)
...
def analyze_image():
    ...
    state = session_data.setdefault(sid, {"welcomed": False, "image_desc": None})
    ...
    if img_b64:
        try:
            ...
            logger.exception("[analyze-image] Error Gemini")
...
def chat():
    ...
    if not state['welcomed']:
        ...
        welcome = (
            ...
            "Por favor, adjunta la imagen relevante y proporciona un breve contexto de lo ocurrido."
        )
        return jsonify({'response': welcome}), 200

    # 2) Verificamos que haya descripción de imagen
    ...
    # 3) Construimos prompt final
    prompt = (
        SYSTEM_INSTRUCTION + "\n\n" +
        "Imagen descrita: " + state['image_desc'] + "\n" +
        "Contexto: " + user_text + "\n\n" +
        "Por favor, proporciona un diagnóstico detallado, un resumen de tu análisis y, si lo deseas, tratamientos adecuados cuando lo solicite."
    )
    logger.info(f"[chat] Prompt a IA: {prompt[:100]}…")
    ...
    try:
        ...
        logger.exception("[chat] Error IA")
